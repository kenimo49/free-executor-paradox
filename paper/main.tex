...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{../results/figures/token-growth.png}
\caption{Free-executor paradox visualized. Each bar is the success-only median of Opus-side (input + cache\_read) tokens for the corresponding task. Even though arm B's executor (Qwen) is free at the token meter, the orchestrator's own context (green bars) is consistently larger than running Opus solo (red). Multipliers in grey annotate the B-over-A and C-over-A ratios per task.}
