...
% Threat-impact color palette
\definecolor{impactHigh}{RGB}{255,205,205}
\definecolor{impactMed}{RGB}{255,236,179}
\definecolor{impactLow}{RGB}{217,234,211}
\newcommand{\impH}{\cellcolor{impactHigh}\textbf{High}}
\newcommand{\impM}{\cellcolor{impactMed}\textbf{Med}}
\newcommand{\impL}{\cellcolor{impactLow}\textbf{Low}}

% Prompt-diff highlight palette (Appendix A)
\definecolor{diffShared}{RGB}{96,96,96}
\definecolor{diffSolo}{RGB}{198,40,40}
\definecolor{diffOrch}{RGB}{27,94,32}
\newcommand{\common}[1]{\textcolor{diffShared}{#1}}
\newcommand{\soloOnly}[1]{\sethlcolor{impactHigh}\hl{#1}}
\newcommand{\orchOnly}[1]{\sethlcolor{impactLow}\hl{#1}}

% Document-wide DOI placeholder (set once, used everywhere)
\newcommand{\paperDOI}{\href{https://doi.org/10.5281/zenodo.20978074}{10.5281/zenodo.20978074}}
...
Table~\ref{tab:threats} summarizes the threats above. The \textbf{Impact} column is a subjective rating of how much each threat could change the paper's headline claim (the free-executor paradox itself, the cost ordering, and the practical guidance), color-coded \colorbox{impactHigh}{High}, \colorbox{impactMed}{Med}, \colorbox{impactLow}{Low}.

\begin{table}[H]
\centering
\small
\begin{tabular}{p{3.3cm} l c p{3.0cm} p{4.0cm}}
\toprule
\textbf{Threat} & \textbf{Type} & \textbf{Impact} & \textbf{Headline effect} & \textbf{Mitigation in place / proposed} \\
\midrule
Prompt asymmetry (solo vs orch.) & Internal & \impH & cross-arm comparability & documented (\S\ref{sec:budget}); future: symmetric ablation \\
Single repository & External & \impH & generalization beyond \texttt{typer} & mechanism is repo-independent (\S5.1); future: \texttt{httpx}, \texttt{click}, \texttt{fastapi} \\
Sampling non-determinism & Internal & \impM & within-cell variance & 95\% bootstrap CI (Table~\ref{tab:summary}); future: pin temperature \\
Iteration cap & Internal & \impM & success-rate column & cost-per-success and CI; future: $n \times 2$ at next cap \\
Small $n$ ($=3$) & Internal & \impM & p-value floor at 0.050 & Cliff's delta (Table~\ref{tab:cliffs}); future: $n \to 10$--$20$ \\
Two cloud models & External & \impM & vendor specificity & structural argument is vendor-agnostic; future: GPT-5, Gemini 2 \\
No parallel delegation & External & \impM & amortization regime & sequential structure documented; future: parallel-fanout extension \\
T3 SHA-256 fingerprint & Construct & \impM & T3 specifically & T3 claim limited to spec-following; future: open-spec variant \\
Summary as proxy for output & Construct & \impM & mechanism scope & we measure summary read-back; future: enforce summary $\leq$ 500 tokens \\
One local executor & External & \impL & executor-LLM specificity & paradox depends on summary volume, not on Qwen; future: 30B-class local \\
Harness as oracle & Construct & \impL & code-quality unmeasured & we claim correctness only, not maintainability; future: LLM-judge complement \\
\bottomrule
\end{tabular}
\caption{Threat matrix sorted by Impact then Type. ``Mitigation in place'' refers to the current paper; ``proposed'' is deferred to future work (\S\ref{sec:future}). The two High-impact threats---prompt asymmetry and single-repository scope---are also the two most concrete next experiments.}
\label{tab:threats}
\end{table}
...
Tested on Ubuntu 22.04 with Python 3.10+ (typer requires \verb|>=3.10|), \texttt{uv} 0.4+, and \texttt{anthropic} Python SDK 0.83+. The Qwen executor uses Ollama 0.4+ on a Tailscale-reachable host running model \texttt{qwen3.5:9b}, exposed via the \texttt{qwen-task --agent} wrapper bundled at \texttt{scripts/qwen-task.sh} (the wrapper itself depends on the \texttt{claw} binary; see README for setup).
...
The four arms use prompts of the same surface form. We render them as a side-by-side diff to make the asymmetry explicit; it is a confounder we discuss in \S\ref{sec:limitations}.

\paragraph{Legend.}
\colorbox{impactHigh}{Red highlight} = solo-only text (arms A and D). \colorbox{impactLow}{Green highlight} = orchestrator-only text (arms B and C). Unhighlighted text is shared verbatim between both prompts.

\subsection{Solo (arms A, D) vs orchestrator (arms B, C) --- unified diff}

\noindent\fbox{\parbox{0.96\textwidth}{\small\ttfamily
You are \soloOnly{a code-fixing agent} \orchOnly{the orchestrator of a two-LLM team. You decide WHAT to do; an executor LLM (which can read files, edit files, and run shell commands) does it for you.}

\medskip
\soloOnly{operating in a sandboxed Python repository.}

\medskip
You have \soloOnly{two} \orchOnly{three} tools:

\medskip
\textbullet~bash: run shell commands in the repo root (120s timeout).

\textbullet~str\_replace\_editor: \soloOnly{view / create / str\_replace / insert files (paths are repo-relative).} \orchOnly{read files yourself with `view` when you need to inspect something before instructing the executor. Avoid using create/str\_replace --- delegate edits to the executor.}

\textbullet~\orchOnly{delegate\_to\_executor: hand off a concrete edit/refactor instruction to the executor. The executor returns a short summary of what it did. Be precise: name files, name functions, name the exact change. The executor is cheaper but weaker than you, so spell out the change rather than asking it to ``figure it out''.}

\medskip
You will be given a task. Iteratively read code, run the harness, fix issues, and re-run until the harness is green. \soloOnly{Be concise in your reasoning; spend tokens on tool calls, not narration.} \orchOnly{Loop: read harness output -> plan -> delegate small chunks -> check harness -> repeat until green.}

\medskip
When you believe the task is complete and the verifier passes, output a single line `DONE' and stop. If you cannot make progress, output `BLOCKED: <reason>' and stop.
}}

\paragraph{What the diff implies.}
The orchestrator prompt explicitly asks the model to \emph{avoid editing directly}; the solo prompt does not constrain editing. This is a fair representation of how the structures are deployed in practice, but it is a confounder for any ``structure alone'' comparison: arm A/D may be advantaged by full editing freedom independent of model capability. Resolving this with a symmetric ablation is item 1 of our internal-validity future work (\S\ref{sec:future}).
